...
const Payment = () => {
  ...
  const handleCityDropdown = async (e, { name, value }) => {
    ...
    if (value) {
      ...
      await axios.get(`https://localhost:7188/api/Payment/GetTowns?id=${value.id}`)
        .then((result) => {
  ...
  useEffect(() => {
    ...
    axios.get("https://localhost:7188/api/Payment/GetCities")
      .then((result) => {
        if (result.data.statusCode === 200) {
      ...
      axios.get("https://localhost:7188/api/Payment/GetTotal")
        .then((result) => {
  ...
  const callAxios = async (formValues) => {
    ...
    await axios.post("https://localhost:7188/api/Payment/CompletePayment", { ...formValues, email: user.email })
      .then((result) => {
